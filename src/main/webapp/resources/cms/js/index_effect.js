...
(function($) {
    $.fn.jCarouselLite = function(o) {
        ...
        return this.each(function() {
			...
			$(tLi).mouseleave(function(){
				ist=false
			})
			
            function go(a) {
				if(ist && o.mousestop){return false}
                if (!b) {
...
$(function(){
	//排行榜切换
	$('.top10_tab_title li').click(function(){
		$(this).addClass('current').siblings().removeClass('current');
		
		var curr=$('.top10_tab_title li').index(this);
		$('.top10_tab_box').addClass('hide').removeClass('block');
		$('.top10_tab_box:eq('+curr+')').addClass('block').removeClass('hide');
	})
	//合作机构切换
	$('.links_title li').click(function(){
		$(this).addClass('current').siblings().removeClass('current');
		
		var curr=$('.links_title li').index(this);
		$('.links_tab').addClass('hide').removeClass('block');
		$('.links_tab:eq('+curr+')').addClass('block').removeClass('hide');
	})
